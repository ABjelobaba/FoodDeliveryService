Vue.component("comment-status", {
    ...
    template: `
        <li class="comment">
            ...
            <div class="comment-status-container" v-bind:name="comment.id" v-if="loggedInRole == 'admin' || loggedInRole == 'manager'">
                
            </div>
            <div v-bind:name="comment.id" class="comment-delete" v-if="loggedInRole == 'admin'" v-on:click="deleteComment">
                <div class="comment-status"><i class="fa fa-trash-o fa-2x" aria-hidden="true"></i></div>    
            </div>
        </li>
             `,
    mounted() {
        if (this.loggedInRole == 'admin' || this.loggedInRole == 'manager') {
